...
import {
  WebsiteAutomatorConfig,
  WebsiteAutomatorConfigSchema,
...
const GlobalEnvConfigSchema = z.object({
  ...
  playwrightConfig: z.object({
    websiteOption: z.record(z.string(), WebsiteAutomatorConfigSchema), // Assuming 'gitlab' is a key and there could be others
  }),
  gitlabProjects: z
    .array(
      z.object({
...
function loadAndValidateAppConfig(): GlobalEnvConfig {
  ...
  try {
    ...
    zodError.errors.forEach((err) => {
      console.error(
        `  - Path: ${err.path.join('.') || 'config root'}, Issue: ${err.message}`,
      )
    })
    throw new Error(
